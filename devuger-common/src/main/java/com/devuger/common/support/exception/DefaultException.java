	...
	public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object obj, Exception ex) {
		...
		if ( ex != null && logger.isErrorEnabled() ) {
			...
			for ( StackTraceElement stack : stacks ) {
				if ( stack.getClassName().startsWith( "com.fastball" ) ) {
		...
		if((contentType != null && contentType.indexOf("json") >= 0) || request.getRequestURI().lastIndexOf(".json") >= 0) {
			if(ex.getClass().equals(HelloException.class)) {
				...
				if(message.startsWith("로그인"))
					modelMap.addAttribute("login", "requied");
				...
				if(ex != null && ex.getMessage() != null && ex.getMessage().startsWith("로그인 정보"))
					modelMap.addAttribute("login", "requied");
